...
const NotificationsPanel = ({ onClose }) => {
  ...
  const unfollowCategory = async (categoryId) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error('Unable to unfollow category');
      }

      loadNotifications();
    } catch (err) {
      console.error('Unfollow category error:', err);
      setError(err.message || 'Unable to unfollow category');
    }
  };
  ...
  const renderMessage = (notification) => {
    const categoryText = notification.category_name ? ` in ${notification.category_name}` : '';
    const entryText = notification.entry_title ? `: ${notification.entry_title}` : '';

    if (notification.type === 'new_product') {
      return `A new product was added${categoryText}${entryText}.`;
    }
    if (notification.type === 'new_experience') {
      return `A new experience was added${categoryText}${entryText}.`;
    }
                  ...
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      {notification.category_id && (
                        <button
                          onClick={() => unfollowCategory(notification.category_id)}
                          className="rounded-full bg-white px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-gray-700 hover:bg-gray-100"
                        >
                          Unfollow
                        </button>
                      )}
